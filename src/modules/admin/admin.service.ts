...
export class AdminService {
  ...
  async getAuditLogs(query: AuditLogQueryDto): Promise<{
    ...
    return {
      data: data.map((log) => {
        const auditLog = log as ApprovalLog & {
          actionTakenByUser?: { fullName: string };
        };

        return {
          approvalLogId: auditLog.id,
          paymentRequestId: Number(auditLog.payment_request_id),
          requestNumber:
            auditLog.payment_request?.request_number ??
            `PRF-${auditLog.payment_request_id}`,
          actionTakenByUserId: Number(auditLog.action_taken_by_user_id),
          actorName: auditLog.actionTakenByUser?.fullName ?? 'Unknown',
          actionTypeId: auditLog.action_type_id,
          previousStatusId: auditLog.previous_status_id ?? null,
          newStatusId: auditLog.new_status_id ?? null,

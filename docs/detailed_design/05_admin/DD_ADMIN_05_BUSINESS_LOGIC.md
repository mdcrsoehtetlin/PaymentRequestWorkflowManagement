...
### 2.6 `getAuditLogById(id: number)`

1. **Validation:** `id` must refer to an existing audit log row.
2. **Logic:**
   - Build a read-only query against `ApprovalLog`.
   - Join related request and user data for the selected row.
   - Return a single immutable audit log detail payload for the right-side
     metadata panel.
3. **Transaction Boundaries:**
   - None. This is a read-only query.
4. **Security Rule:**
   - The audit log detail endpoint must remain read-only and cannot mutate
     historical records.

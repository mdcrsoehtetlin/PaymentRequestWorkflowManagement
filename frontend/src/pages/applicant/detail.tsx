...
const PaymentRequestDetail: React.FC = () => {
  ...
  const handleApproverSubmit = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Failed to submit to Approver');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEditToggle = () => {
    if (!isEditing) {
      setEditData({
        ...
        breakdowns: data.breakdowns ? data.breakdowns.map((b: any) => ({ ...b })) : [],
      });
    }
  ...
  const rejectionLog = data.logs?.find((log: any) => log.new_status_id === 5 || log.new_status_id === 9);
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {(isEditing ? editData.breakdowns : data.breakdowns)?.map((item: any, i: number) => (
              ...
              <div className="p-5 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-slate-900">
                  {isEditing ? 
                    `$${editData.breakdowns.reduce((sum: number, b: any) => sum + Number(b.amount), 0).toFixed(2)}` : 

...
export interface ApproverColumn<T> {
  key: string;
  header: string;
  sortable?: boolean;
...
interface ApproverDataTableProps<T> {
  columns: ApproverColumn<T>[];
  data: T[];
  isLoading?: boolean;
  emptyMessage?: string;
...
export function ApproverDataTable<T extends Record<string, unknown>>({
  columns,
  data,
  isLoading = false,
      ...
      {pagination && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50">
          <div className="flex items-center text-sm text-slate-500">
            ...
            of <span className="mx-1 font-medium text-slate-900">{pagination.totalItems}</span> records
          </div>
